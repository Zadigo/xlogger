<template>
  <section id="admin" class="flex flex-1" :class="[variant === 'inset' && 'bg-neutral-50 dark:bg-neutral-950', side === 'right' && 'flex-row-reverse']">
    <u-sidebar v-model:open="open" :variant="variant" :collapsible="collapsible" :side="side" :ui="{ container: 'h-full' }">
      <template #header>
        <u-icon name="i-logos-nuxt-icon" class="size-8" />
      </template>

      <u-navigation-menu :items="items" orientation="vertical" :ui="{ link: 'p-1.5 overflow-hidden' }" />
    </u-sidebar>
